...
function defaultConfig(): PetAgentConfig {
...
export async function configure(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" petagent configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `petagent onboard` first.");
    p.outro("");
    return;
  }

  let config: PetAgentConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because PETAGENT_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
