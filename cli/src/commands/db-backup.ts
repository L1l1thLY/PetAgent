...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@petagent/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://petagent:petagent@127.0.0.1:${port}/petagent`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" petagent db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolvePetAgentInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "petagent";
