...
export const DEFAULT_WORKTREE_HOME = "~/.petagent-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".petagent");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: PetAgentConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "petagent",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    PETAGENT_HOME: paths.homeDir,
    PETAGENT_INSTANCE_ID: paths.instanceId,
    PETAGENT_CONFIG: paths.configPath,
    PETAGENT_CONTEXT: paths.contextPath,
    PETAGENT_IN_WORKTREE: "true",
    ...(branding?.name ? { PETAGENT_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { PETAGENT_WORKTREE_COLOR: branding.color } : {}),
  };
}
