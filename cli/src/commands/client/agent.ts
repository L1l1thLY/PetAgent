...
function buildAgentEnvExports(input: {
  ...
  return [
    `export PETAGENT_API_URL='${escaped(input.apiBase)}'`,
    `export PETAGENT_COMPANY_ID='${escaped(input.companyId)}'`,
    `export PETAGENT_AGENT_ID='${escaped(input.agentId)}'`,
    `export PETAGENT_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local PetAgent skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing PetAgent skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolvePetAgentSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local PetAgent skills directory. Expected ./skills in the repo checkout.",
              );
            }
