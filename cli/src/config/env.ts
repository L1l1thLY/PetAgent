...
const JWT_SECRET_ENV_KEY = "PETAGENT_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by PetAgent CLI commands",
...
export function resolvePetAgentEnvFile(configPath?: string): string {
...
export function loadPetAgentEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergePetAgentEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readPetAgentEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writePetAgentEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergePetAgentEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readPetAgentEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writePetAgentEnvEntries(next, filePath);
  return next;
}
