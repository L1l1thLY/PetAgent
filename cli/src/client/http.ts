...
export class PetAgentApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-petagent-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  lines.push(
    "",
    "This usually means the PetAgent server is not running, the configured URL is wrong, or the request is being blocked before it reaches PetAgent.",
    "",
    "Try:",
