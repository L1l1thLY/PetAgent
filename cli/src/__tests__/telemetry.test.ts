...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "petagent-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.PETAGENT_HOME = path.join(root, "home");
    process.env.PETAGENT_INSTANCE_ID = "telemetry-test";
