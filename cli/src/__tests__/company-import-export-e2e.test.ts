...
function createServerEnv(configPath: string, port: number, connectionString: string) {
  ...
  env.PETAGENT_CONFIG = configPath;
  env.DATABASE_URL = connectionString;
  env.HOST = "127.0.0.1";
  env.PORT = String(port);
  env.SERVE_UI = "false";
  env.PETAGENT_DB_BACKUP_ENABLED = "false";
  env.HEARTBEAT_SCHEDULER_ENABLED = "false";
  env.PETAGENT_MIGRATION_AUTO_APPLY = "true";
  env.PETAGENT_UI_DEV_MIDDLEWARE = "false";
...
function createCliEnv() {
  ...
  delete env.HOST;
  delete env.SERVE_UI;
  delete env.PETAGENT_DB_BACKUP_ENABLED;
  delete env.HEARTBEAT_SCHEDULER_ENABLED;
  delete env.PETAGENT_MIGRATION_AUTO_APPLY;
  delete env.PETAGENT_UI_DEV_MIDDLEWARE;
  return env;
}
...
async function runCliJson<T>(args: string[], opts: { apiBase: string; configPath: string }) {
  ...
  const result = await execFileAsync(
    "pnpm",
    ["--silent", "petagent", ...args, "--api-base", opts.apiBase, "--config", opts.configPath, "--json"],
    {
      cwd: repoRoot,
      env: createCliEnv(),
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 30_000) {
    if (child.exitCode !== null) {
      throw new Error(
        `petagent run exited before healthcheck succeeded.\nstdout:\n${output.stdout.join("")}\nstderr:\n${output.stderr.join("")}`,
      );
    }
...
describeEmbeddedPostgres("petagent company import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
  ...
  beforeAll(async () => {
    tempRoot = mkdtempSync(path.join(os.tmpdir(), "petagent-company-cli-e2e-"));
    configPath = path.join(tempRoot, "config", "config.json");
    exportDir = path.join(tempRoot, "exported-company");

    tempDb = await startEmbeddedPostgresTestDatabase("petagent-company-cli-db-");
    ...
    const child = spawn(
      "pnpm",
      ["petagent", "run", "--config", configPath],
      {
        cwd: repoRoot,
  ...
  it("exports a company package and imports it into new and existing companies", async () => {
    ...
    expect(readFileSync(path.join(exportDir, ".petagent.yaml"), "utf8")).toContain('schema: "petagent/v1"');
    ...
    writeFileSync(zipPath, createStoredZipArchive(portableFiles, "petagent-demo"));
