/**
 * esbuild configuration for building the petagent CLI for npm.
 *
 * Bundles all workspace packages (@petagent/*) into a single file.
...
// to npm and resolved at runtime (e.g. @petagent/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@petagent/")) {
      externals.add(name);
    }
  }
