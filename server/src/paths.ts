...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".petagent", PAPERCLIP_CONFIG_BASENAME);
...
export function resolvePetAgentEnvPath(overrideConfigPath?: string): string {
  return path.resolve(path.dirname(resolvePetAgentConfigPath(overrideConfigPath)), PAPERCLIP_ENV_FILENAME);
}
