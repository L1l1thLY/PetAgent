...
import { getAdapterSessionManagement } from "@petagent/adapter-utils";
import {
...
} from "@petagent/adapter-claude-local/server";
import { agentConfigurationDoc as claudeAgentConfigurationDoc, models as claudeModels } from "@petagent/adapter-claude-local";
import {
...
import { agentConfigurationDoc as codexAgentConfigurationDoc, models as codexModels } from "@petagent/adapter-codex-local";
import {
...
import { agentConfigurationDoc as cursorAgentConfigurationDoc, models as cursorModels } from "@petagent/adapter-cursor-local";
import {
...
} from "@petagent/adapter-gemini-local/server";
import { agentConfigurationDoc as geminiAgentConfigurationDoc, models as geminiModels } from "@petagent/adapter-gemini-local";
import {
  ...
  listOpenCodeModels,
} from "@petagent/adapter-opencode-local/server";
import {
  agentConfigurationDoc as openCodeAgentConfigurationDoc,
  models as openCodeModels,
} from "@petagent/adapter-opencode-local";
import {
  execute as openclawGatewayExecute,
  testEnvironment as openclawGatewayTestEnvironment,
} from "@petagent/adapter-openclaw-gateway/server";
import {
  agentConfigurationDoc as openclawGatewayAgentConfigurationDoc,
  models as openclawGatewayModels,
} from "@petagent/adapter-openclaw-gateway";
...
import {
  ...
  listPiModels,
} from "@petagent/adapter-pi-local/server";
import {
  agentConfigurationDoc as piAgentConfigurationDoc,
} from "@petagent/adapter-pi-local";
import {
...
} from "hermes-petagent-adapter/server";
import {
  agentConfigurationDoc as hermesAgentConfigurationDoc,
  models as hermesModels,
} from "hermes-petagent-adapter";
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[petagent] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[petagent] Failed to load external adapters:", err);
  }
})();
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[petagent] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[petagent] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}
