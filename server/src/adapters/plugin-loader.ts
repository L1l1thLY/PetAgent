...
function extractUiParserSource(
  ...
  const contractVersion = pkg.petagent?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no petagent.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
