...
import type { PetAgentConfig } from "@petagent/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by PetAgent worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.PETAGENT_IN_WORKTREE !== "true") return null;

  const configPath = resolvePetAgentConfigPath(overrideConfigPath);
  ...
  const worktreeName =
    nonEmpty(persistedEnv.PETAGENT_WORKTREE_NAME) ??
    nonEmpty(env.PETAGENT_WORKTREE_NAME) ??
    path.basename(worktreeRoot);
  const instanceId =
    nonEmpty(persistedEnv.PETAGENT_INSTANCE_ID) ??
    nonEmpty(env.PETAGENT_INSTANCE_ID) ??
    sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(persistedEnv.PETAGENT_HOME) ??
      nonEmpty(env.PETAGENT_HOME) ??
      nonEmpty(env.PETAGENT_WORKTREES_DIR) ??
      "~/.petagent-worktrees",
  );
...
function writeConfigFile(configPath: string, config: PetAgentConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".petagent", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as PetAgentConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: PetAgentConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: PetAgentConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.PETAGENT_HOME = context.homeDir;
  process.env.PETAGENT_INSTANCE_ID = context.instanceId;
  process.env.PETAGENT_CONFIG = context.configPath;
  process.env.PETAGENT_CONTEXT = context.contextPath;
  process.env.PETAGENT_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as PetAgentConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    PETAGENT_HOME: context.homeDir,
    PETAGENT_INSTANCE_ID: context.instanceId,
    PETAGENT_CONFIG: context.configPath,
    PETAGENT_CONTEXT: context.contextPath,
    PETAGENT_IN_WORKTREE: "true",
    PETAGENT_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: PetAgentConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as PetAgentConfig;
  } catch {
    return;
  }
