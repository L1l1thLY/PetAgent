...
const PETAGENT_ENV_FILE_PATH = resolvePetAgentEnvPath();
if (existsSync(PETAGENT_ENV_FILE_PATH)) {
  loadDotenv({ path: PETAGENT_ENV_FILE_PATH, override: false, quiet: true });
}

const CWD_ENV_PATH = resolve(process.cwd(), ".env");
const isSameFile = existsSync(CWD_ENV_PATH) && existsSync(PETAGENT_ENV_FILE_PATH)
  ? realpathSync(CWD_ENV_PATH) === realpathSync(PETAGENT_ENV_FILE_PATH)
  : CWD_ENV_PATH === PETAGENT_ENV_FILE_PATH;
...
function detectTailnetBindHost(): string | undefined {
  const explicit = process.env.PETAGENT_TAILNET_BIND_HOST?.trim();
...
export function loadConfig(): Config {
  ...
  const strictModeFromEnv = process.env.PETAGENT_SECRETS_STRICT_MODE;
  const secretsStrictMode =
    strictModeFromEnv !== undefined
      ? strictModeFromEnv === "true"
      : (fileSecrets?.strictMode ?? false);

  const providerFromEnvRaw = process.env.PETAGENT_SECRETS_PROVIDER;
  ...
  const storageProviderFromEnvRaw = process.env.PETAGENT_STORAGE_PROVIDER;
  ...
  const storageS3Bucket = process.env.PETAGENT_STORAGE_S3_BUCKET ?? fileStorage?.s3?.bucket ?? "petagent";
  const storageS3Region = process.env.PETAGENT_STORAGE_S3_REGION ?? fileStorage?.s3?.region ?? "us-east-1";
  const storageS3Endpoint = process.env.PETAGENT_STORAGE_S3_ENDPOINT ?? fileStorage?.s3?.endpoint ?? undefined;
  const storageS3Prefix = process.env.PETAGENT_STORAGE_S3_PREFIX ?? fileStorage?.s3?.prefix ?? "";
  const storageS3ForcePathStyle =
    process.env.PETAGENT_STORAGE_S3_FORCE_PATH_STYLE !== undefined
      ? process.env.PETAGENT_STORAGE_S3_FORCE_PATH_STYLE === "true"
      : (fileStorage?.s3?.forcePathStyle ?? false);
  const feedbackExportBackendUrl =
    process.env.PETAGENT_FEEDBACK_EXPORT_BACKEND_URL?.trim() ||
    process.env.PETAGENT_TELEMETRY_BACKEND_URL?.trim() ||
    undefined;
  const feedbackExportBackendToken =
    process.env.PETAGENT_FEEDBACK_EXPORT_BACKEND_TOKEN?.trim() ||
    process.env.PETAGENT_TELEMETRY_BACKEND_TOKEN?.trim() ||
    undefined;

  const deploymentModeFromEnvRaw = process.env.PETAGENT_DEPLOYMENT_MODE;
  const deploymentModeFromEnv =
    deploymentModeFromEnvRaw && DEPLOYMENT_MODES.includes(deploymentModeFromEnvRaw as DeploymentMode)
      ? (deploymentModeFromEnvRaw as DeploymentMode)
      : null;
  const deploymentMode: DeploymentMode = deploymentModeFromEnv ?? fileConfig?.server.deploymentMode ?? "local_trusted";
  const deploymentExposureFromEnvRaw = process.env.PETAGENT_DEPLOYMENT_EXPOSURE;
  ...
  const bindFromEnvRaw = process.env.PETAGENT_BIND;
  const bindFromEnv =
  ...
  const customBindHost = process.env.PETAGENT_BIND_HOST ?? fileConfig?.server.customBindHost;
  const authBaseUrlModeFromEnvRaw = process.env.PETAGENT_AUTH_BASE_URL_MODE;
  const authBaseUrlModeFromEnv =
    authBaseUrlModeFromEnvRaw &&
    AUTH_BASE_URL_MODES.includes(authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      ? (authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      : null;
  const publicUrlFromEnv = process.env.PETAGENT_PUBLIC_URL;
  ...
  const disableSignUpFromEnv = process.env.PETAGENT_AUTH_DISABLE_SIGN_UP;
  const authDisableSignUp: boolean =
    disableSignUpFromEnv !== undefined
      ? disableSignUpFromEnv === "true"
      : (fileConfig?.auth?.disableSignUp ?? false);
  const allowedHostnamesFromEnvRaw = process.env.PETAGENT_ALLOWED_HOSTNAMES;
  ...
  const companyDeletionEnvRaw = process.env.PETAGENT_ENABLE_COMPANY_DELETION;
  const companyDeletionEnabled =
    companyDeletionEnvRaw !== undefined
      ? companyDeletionEnvRaw === "true"
      : deploymentMode === "local_trusted";
  const databaseBackupEnabled =
    process.env.PETAGENT_DB_BACKUP_ENABLED !== undefined
      ? process.env.PETAGENT_DB_BACKUP_ENABLED === "true"
      : (fileDatabaseBackup?.enabled ?? true);
  const databaseBackupIntervalMinutes = Math.max(
    1,
    Number(process.env.PETAGENT_DB_BACKUP_INTERVAL_MINUTES) ||
      fileDatabaseBackup?.intervalMinutes ||
      60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    Number(process.env.PETAGENT_DB_BACKUP_RETENTION_DAYS) ||
      fileDatabaseBackup?.retentionDays ||
      7,
  );
  const databaseBackupDir = resolveHomeAwarePath(
    process.env.PETAGENT_DB_BACKUP_DIR ??
  ...
  return {
    ...
    uiDevMiddleware: process.env.PETAGENT_UI_DEV_MIDDLEWARE === "true",
    secretsProvider,
    secretsStrictMode,
