...
import { readPetAgentSkillSyncPreference } from "@petagent/adapter-utils/server-utils";
import { claudeConfigDir, parseClaudeStreamJson } from "@petagent/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@petagent/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@petagent/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolvePetAgentInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "petagent-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "petagent-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "petagent-feedback-v1";
const FEEDBACK_DESTINATION = "petagent_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "petagent_run" &&
    file.source !== "petagent_run_events" &&
    file.source !== "petagent_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.PETAGENT_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readPetAgentSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let petagentRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      petagentRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "petagent/run.json",
        contentType: "application/json",
        source: "petagent_run",
        contents: `${JSON.stringify(petagentRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.petagentRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "petagent/run-events.json",
        contentType: "application/json",
        source: "petagent_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "petagent/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "petagent_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.petagentRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
