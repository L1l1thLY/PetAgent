 ...
 *    (`~/.petagent/plugins/`) and `node_modules` for packages matching
 *    the `petagent-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published PetAgent plugins.
 * Packages matching this pattern are considered PetAgent plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "petagent-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: PetAgentPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.petagent/plugins/ local directory
  | "npm"               // npm packages matching petagent-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: PetAgentPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the petagent-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "petagent-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: PetAgentPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover PetAgent plugins installed as npm packages in the current
   * Node.js environment matching the "petagent-plugin-*" naming convention.
   *
   ...
   * the "petagentPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a PetAgent plugin.
   * Throws if the package is a PetAgent plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<PetAgentPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: PetAgentPluginManifestV1;
    newManifest: PetAgentPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the PetAgent plugin naming convention.
 * Accepts both the "petagent-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @petagent/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "petagentPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const petagentPlugin = pkgJson["petagentPlugin"];
  if (
    petagentPlugin !== null &&
    typeof petagentPlugin === "object" &&
    !Array.isArray(petagentPlugin)
  ) {
    const manifestRelPath = (petagentPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: PetAgentPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumPetAgentVersion;
}
 ...
 *   packageName: "petagent-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: PetAgentPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as PetAgentPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a PetAgent plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<PetAgentPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a PetAgent plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasPetAgentPlugin = "petagentPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasPetAgentPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<PetAgentPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: PetAgentPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @petagent/shared exports). Run those workers through
