...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@petagent/db";
 ...
 *
 * This implementation delegates to the core PetAgent domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@petagent/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
