 ...
 * ~/.petagent/adapter-plugins.json. This is the source of truth for which
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "droid-petagent-adapter") */
  packageName: string;
...
const PETAGENT_DIR = path.join(os.homedir(), ".petagent");
const ADAPTER_PLUGINS_DIR = path.join(PETAGENT_DIR, "adapter-plugins");
const ADAPTER_PLUGINS_STORE_PATH = path.join(PETAGENT_DIR, "adapter-plugins.json");
const ADAPTER_SETTINGS_PATH = path.join(PETAGENT_DIR, "adapter-settings.json");
...
function ensureDirs(): void {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "petagent-adapter-plugins",
      version: "0.0.0",
      private: true,
      description: "Managed directory for PetAgent external adapter plugins. Do not edit manually.",
    }, null, 2) + "\n");
  }
}
