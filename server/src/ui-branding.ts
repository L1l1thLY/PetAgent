const FAVICON_BLOCK_START = "<!-- PETAGENT_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- PETAGENT_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- PETAGENT_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- PETAGENT_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.PETAGENT_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.PETAGENT_WORKTREE_NAME) ?? nonEmpty(env.PETAGENT_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.PETAGENT_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="petagent-worktree-enabled" content="true" />',
    `<meta name="petagent-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="petagent-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="petagent-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
