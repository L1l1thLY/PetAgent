...
describe("gemini local skill sync", () => {
  ...
  it("reports configured PetAgent skills and installs them into the Gemini skills home", async () => {
    const home = await makeTempDir("petagent-gemini-skill-sync-");
    ...
    const after = await syncGeminiSkills(ctx, [petagentKey]);
    expect(after.entries.find((entry) => entry.key === petagentKey)?.state).toBe("installed");
    expect((await fs.lstat(path.join(home, ".gemini", "skills", "petagent"))).isSymbolicLink()).toBe(true);
  });

  it("keeps required bundled PetAgent skills installed even when the desired set is emptied", async () => {
    const home = await makeTempDir("petagent-gemini-skill-prune-");
    cleanupDirs.add(home);
    ...
    await syncGeminiSkills(configuredCtx, [petagentKey]);
