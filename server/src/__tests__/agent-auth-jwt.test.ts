...
describe("agent local JWT", () => {
  const secretEnv = "PETAGENT_AGENT_JWT_SECRET";
  const betterAuthSecretEnv = "BETTER_AUTH_SECRET";
  const ttlEnv = "PETAGENT_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "PETAGENT_AGENT_JWT_ISSUER";
  const audienceEnv = "PETAGENT_AGENT_JWT_AUDIENCE";
  ...
  it("falls back to BETTER_AUTH_SECRET when PETAGENT_AGENT_JWT_SECRET is absent", () => {
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "petagent";
    process.env[audienceEnv] = "petagent-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
