...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    ...
    await fs.writeFile(
      ...
      [
        ...
        "PETAGENT_WORKTREE_NAME=PAP-884-ai-commits-component",
        "PETAGENT_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.PETAGENT_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.PETAGENT_WORKTREES_DIR = isolatedHome;
    delete process.env.PETAGENT_HOME;
    delete process.env.PETAGENT_INSTANCE_ID;
    delete process.env.PETAGENT_CONFIG;
    delete process.env.PETAGENT_CONTEXT;
    ...
    expect(repairedEnv).toContain(`PETAGENT_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('PETAGENT_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`PETAGENT_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`PETAGENT_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('PETAGENT_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.PETAGENT_HOME).toBe(isolatedHome);
    expect(process.env.PETAGENT_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });
  ...
  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "# PetAgent environment variables",
        "PETAGENT_IN_WORKTREE=true",
        "PETAGENT_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.PETAGENT_IN_WORKTREE = "true";
    process.env.PETAGENT_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.PETAGENT_WORKTREES_DIR = isolatedHome;
  ...
  it("does not persist transient runtime home overrides over repo-local worktree env", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "# PetAgent environment variables",
        `PETAGENT_HOME=${JSON.stringify(isolatedHome)}`,
        `PETAGENT_INSTANCE_ID=${JSON.stringify(instanceId)}`,
        `PETAGENT_CONFIG=${JSON.stringify(configPath)}`,
        `PETAGENT_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`,
        'PETAGENT_IN_WORKTREE="true"',
        'PETAGENT_WORKTREE_NAME="PAP-989-multi-user-implementation-using-plan-from-pap-958"',
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.PETAGENT_IN_WORKTREE = "true";
    process.env.PETAGENT_WORKTREE_NAME = "PAP-989-multi-user-implementation-using-plan-from-pap-958";
    process.env.PETAGENT_HOME = transientHome;
    process.env.PETAGENT_INSTANCE_ID = instanceId;
    process.env.PETAGENT_CONFIG = configPath;
    ...
    expect(repairedEnv).toContain(`PETAGENT_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).not.toContain(`PETAGENT_HOME=${JSON.stringify(transientHome)}`);
    expect(process.env.PETAGENT_HOME).toBe(isolatedHome);
  });
  ...
  it("persists runtime-selected worktree ports back into config", async () => {
    ...
    process.chdir(worktreeRoot);
    process.env.PETAGENT_IN_WORKTREE = "true";
    process.env.PETAGENT_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.PETAGENT_HOME = isolatedHome;
    process.env.PETAGENT_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.PETAGENT_CONFIG = configPath;
