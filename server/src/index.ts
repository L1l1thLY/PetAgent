...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.PETAGENT_SECRETS_PROVIDER === undefined) {
    process.env.PETAGENT_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.PETAGENT_SECRETS_STRICT_MODE === undefined) {
    process.env.PETAGENT_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.PETAGENT_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.PETAGENT_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.PETAGENT_MIGRATION_AUTO_APPLY === "true") return true;
    if (process.env.PETAGENT_MIGRATION_PROMPT === "never") return false;
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set PETAGENT_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@petagent.local";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.PETAGENT_EMBEDDED_POSTGRES_VERBOSE === "true";
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://petagent:petagent@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "petagent");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://petagent:petagent@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "petagent");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: petagent");
    }
  
    const embeddedConnectionString = `postgres://petagent:petagent@127.0.0.1:${port}/petagent`;
  ...
  const runtimeApiHost =
    runtimeListenHost === "0.0.0.0" || runtimeListenHost === "::"
      ? "localhost"
      : runtimeListenHost;
  process.env.PETAGENT_LISTEN_HOST = runtimeListenHost;
  process.env.PETAGENT_LISTEN_PORT = String(listenPort);
  if (!process.env.PETAGENT_API_URL) {
    process.env.PETAGENT_API_URL = `http://${runtimeApiHost}:${listenPort}`;
  }
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.PETAGENT_OPEN_ON_LISTEN === "true") {
  ...
  return {
    ...
    host: config.host,
    listenPort,
    apiUrl: process.env.PETAGENT_API_URL!,
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "PetAgent server failed to start");
    process.exit(1);
  });
}
