...
interface AdapterInstallRequest {
  /** npm package name (e.g., "droid-petagent-adapter") or local path */
  packageName: string;
...
export function adapterRoutes() {
  ...
  router.post("/adapters/install", async (req, res) => {
    ...
    // e.g. "@henkey/hermes-petagent-adapter@0.3.0" → packageName + version
  ...
  // without modifying PetAgent's source code.
  //
