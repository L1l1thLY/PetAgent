...
import type { DeploymentExposure, DeploymentMode, PermissionKey } from "@petagent/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm petagent allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/petagent-claimed-api-key.json and load PETAGENT_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      inviteMessage: extractInviteMessage(invite),
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    And set the PETAGENT_API_KEY and PETAGENT_API_URL in your environment variables as specified here:
    ...
    Then set PETAGENT_API_KEY and PETAGENT_API_URL from the saved token field for every heartbeat run.
    ...
    Be sure to prepend your PETAGENT_API_URL to the top of your skill and note the path to your PETAGENT_API_URL
