...
import { resolvePetAgentConfigPath, resolvePetAgentEnvPath } from "./paths.js";
import type { BindMode, DeploymentExposure, DeploymentMode } from "@petagent/shared";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.PETAGENT_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.PETAGENT_AGENT_JWT_SECRET === "string" ? parsed.PETAGENT_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm petagent onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolvePetAgentEnvPath();
