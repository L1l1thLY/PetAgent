...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://petagent:petagent@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "petagent");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://petagent:petagent@127.0.0.1:${preferredPort}/petagent`,
  ...
  if (runningPid) {
    ...
    return {
      connectionString: `postgres://petagent:petagent@127.0.0.1:${port}/petagent`,
  ...
  const adminConnectionString = `postgres://petagent:petagent@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "petagent");

  return {
    connectionString: `postgres://petagent:petagent@127.0.0.1:${selectedPort}/petagent`,
