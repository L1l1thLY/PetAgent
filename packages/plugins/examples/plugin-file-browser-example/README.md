...
Example PetAgent plugin that demonstrates:
...
```bash
pnpm --filter @petagentai/plugin-file-browser-example build
pnpm petagentai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm petagentai plugin uninstall petagent-file-browser-example --force
```
...
- Optional: use `petagent-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
