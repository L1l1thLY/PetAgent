...
type PetAgentSprite = (typeof PETAGENT_SPRITES)[number];
...
function spriteSize(sprite: PetAgentSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = PETAGENT_SPRITES[(baseCol + baseRow) % PETAGENT_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = PETAGENT_SPRITES[Math.floor(Math.random() * PETAGENT_SPRITES.length)]!;
