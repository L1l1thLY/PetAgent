...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".petagent-sdk");
...
/**
 * Generate a complete PetAgent plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A PetAgent plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "petagent-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["petagent", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@petagent/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@petagent/plugin-sdk": sdkDependency,
...
import { createPluginBundlerPresets } from "@petagent/plugin-sdk/bundlers";
...
`,
  ...
  writeFile(
    ...
    `import { definePlugin, runWorker } from "@petagent/plugin-sdk";
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@petagent/plugin-sdk/ui";
...
import { createTestHarness } from "@petagent/plugin-sdk/testing";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@petagent/plugin-sdk\` and \`@petagent/shared\` from a local PetAgent checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.petagent-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
- \`pnpm build\` uses esbuild presets from \`@petagent/plugin-sdk/bundlers\`.
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-petagent-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <petagent-sdk-path>]");
    process.exit(1);
  }
