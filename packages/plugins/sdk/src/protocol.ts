 ...
 * channel.
 *
 * The PetAgent plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@petagent/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all PetAgent messages.
 */
...
/**
 * PetAgent plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this PetAgent instance. */
    instanceId: string;
    /** Semver version of the running PetAgent host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
