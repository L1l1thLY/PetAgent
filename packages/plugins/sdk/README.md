# `@petagentai/plugin-sdk`

Official TypeScript SDK for PetAgent plugin authors.

- **Worker SDK:** `@petagentai/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@petagentai/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@petagentai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@petagentai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@petagentai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@petagentai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@petagentai/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@petagentai/plugin-sdk/ui/hooks` | Hooks only |
| `@petagentai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@petagentai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@petagentai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@petagentai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@petagentai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@petagentai/plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @petagentai/plugin-sdk
```
...
- Plugin UI bundles run as same-origin JavaScript inside the main PetAgent app. They can call ordinary PetAgent HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the PetAgent instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@petagentai/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@petagentai/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core PetAgent information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@petagentai/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@petagentai/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@petagentai/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@petagentai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@petagentai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@petagentai/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@petagentai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@petagentai/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@petagentai/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@petagentai/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@petagentai/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@petagentai/plugin-sdk/bundlers";
...
```bash
petagent-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@petagentai/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__petagent__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__petagent__/events` streams `reload` SSE events on UI build changes
