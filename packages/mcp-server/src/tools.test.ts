...
describe("petagent MCP tools", () => {
  ...
  it("adds auth headers and run id to mutating requests", async () => {
    ...
    const tool = getTool("petagentUpdateIssue");
    await tool.execute({
      issueId: "PAP-1135",
      status: "done",
    });
    ...
    expect((init.headers as Record<string, string>)["X-PetAgent-Run-Id"]).toBe(
  ...
  it("uses default company id for company-scoped list tools", async () => {
    ...
    const tool = getTool("petagentListIssues");
  ...
  it("uses default agent id for checkout requests", async () => {
    ...
    const tool = getTool("petagentCheckoutIssue");
    await tool.execute({
      issueId: "PAP-1135",
    });
  ...
  it("defaults issue document format to markdown", async () => {
    ...
    const tool = getTool("petagentUpsertIssueDocument");
    await tool.execute({
      issueId: "PAP-1135",
      key: "plan",
      body: "# Updated",
  ...
  it("creates approvals with the expected company-scoped payload", async () => {
    ...
    const tool = getTool("petagentCreateApproval");
    await tool.execute({
      type: "hire_agent",
  ...
  it("rejects invalid generic request paths", async () => {
    ...
    const tool = getTool("petagentApiRequest");
    const response = await tool.execute({
      method: "GET",
      path: "issues",
    });
