...
- `PETAGENT_API_URL` - PetAgent base URL, for example `http://localhost:3100`
- `PETAGENT_API_KEY` - bearer token used for `/api` requests
- `PETAGENT_COMPANY_ID` - optional default company for company-scoped tools
- `PETAGENT_AGENT_ID` - optional default agent for checkout helpers
- `PETAGENT_RUN_ID` - optional run id forwarded on mutating requests
