...
import type { AdapterModel } from "@petagent/adapter-utils";
import { asString, runChildProcess } from "@petagent/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.PETAGENT_PI_COMMAND === "string" &&
    process.env.PETAGENT_PI_COMMAND.trim().length > 0
      ? process.env.PETAGENT_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["PETAGENT_", "npm_", "NPM_"] as const;
