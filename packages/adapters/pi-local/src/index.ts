...
Use when:
- You want PetAgent to run Pi (the AI coding agent) locally as the agent runtime
...
- PetAgent requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/petagents/ and resumed with --session.
