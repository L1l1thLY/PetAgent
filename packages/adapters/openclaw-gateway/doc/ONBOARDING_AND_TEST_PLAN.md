...
## Scope
This plan is now **gateway-only**. PetAgent supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses PetAgent skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a PetAgent task.
...
```bash
pnpm --filter @petagentai/server test:run -- openclaw-gateway
pnpm --filter @petagentai/server typecheck
pnpm --filter @petagentai/ui typecheck
pnpm --filter petagentai typecheck
```
