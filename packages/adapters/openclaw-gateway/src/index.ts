...
Use when:
- You want PetAgent to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the PetAgent server.
...
- petagentApiUrl (string, optional): absolute PetAgent base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/petagent-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default petagent)

Standard outbound payload additions:
- petagent (object): standardized PetAgent context added to every gateway agent request
- petagent.workspace (object, optional): resolved execution workspace for this run
- petagent.workspaces (array, optional): additional workspace hints PetAgent exposed to the run
- petagent.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
