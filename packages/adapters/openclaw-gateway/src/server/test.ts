...
import { asString, parseObject } from "@petagent/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the PetAgent server host.",
        });
      }
    } catch (err) {
      checks.push({
