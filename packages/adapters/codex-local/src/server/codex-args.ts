import { asBoolean, asString, asStringArray } from "@petagent/adapter-utils/server-utils";
import {
  ...
  return {
    ...
    fastModeIgnoredReason:
      fastModeRequested && !fastModeApplied
        ? `Configured fast mode is currently only supported on ${formatFastModeSupportedModels()}; PetAgent will ignore it for model ${model || "(default)"}.`
        : null,
  };
}
