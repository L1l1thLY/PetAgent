...
- If instructionsFilePath is configured, PetAgent prepends that file's contents to the stdin prompt on every run.
- Codex exec automatically applies repo-scoped AGENTS.md instructions from the active workspace. PetAgent cannot suppress that discovery in exec mode, so repo AGENTS.md files may still apply even when you only configured an explicit instructionsFilePath.
- PetAgent injects desired local skills into the effective CODEX_HOME/skills/ directory at execution time so Codex can discover "$petagent" and related skills without polluting the project working directory. In managed-home mode (the default) this is ~/.petagent/instances/<id>/companies/<companyId>/codex-home/skills/; when CODEX_HOME is explicitly overridden in adapter config, that override is used instead.
