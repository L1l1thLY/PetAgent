...
PetAgent companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .petagent.yaml     # Adapter config, env inputs, routines
```
...
- **.petagent.yaml** holds PetAgent-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
petagentai company export <company-id> --out ./my-export
```
...
petagentai company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
petagentai company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
petagentai company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.petagent.yaml`
...
```sh
# From a local folder
petagentai company import ./my-export

# From a GitHub URL
petagentai company import https://github.com/org/repo

# From a GitHub subfolder
petagentai company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
petagentai company import org/repo
petagentai company import org/repo/companies/acme
```
...
If `--target` is not specified, PetAgent infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
petagentai company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
petagentai company import org/company-templates/engineering-team \
  --target new \
...
```sh
petagentai company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
petagentai company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
petagentai company import ./package \
  --target new \
  --yes \
  --json
```
...
PetAgent supports several GitHub URL formats:
