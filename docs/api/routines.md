...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-PetAgent-Signature` + `X-PetAgent-Timestamp` header pair matching the trigger's signing mode.
