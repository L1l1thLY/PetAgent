...
Audience: Operators setting up and running agents in PetAgent

## 1. What this system does

Agents in PetAgent do not run continuously.  
...
- `hermes_local`: runs your local `hermes` CLI (`hermes-petagent-adapter`)
...
- `droid_local`: runs your local Factory Droid CLI (`@henkey/droid-petagent-adapter`)

For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), PetAgent assumes the CLI is already installed and authenticated on the host machine.
...
PetAgent stores session IDs for resumable adapters.
...
PetAgent pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. PetAgent surfaces this as a warning in environment tests, not a hard error.
