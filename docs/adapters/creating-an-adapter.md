...
Build a custom adapter to connect PetAgent to any agent runtime.
...
|---|---|---|
| Source | Inside `petagent-fork` | Separate npm package |
| Distribution | Ships with PetAgent | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying PetAgent's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@petagentai/adapter-utils/server-utils`
2. Build environment with `buildPetAgentEnv(agent)` plus context vars
...
For built-in adapters registered in PetAgent's source:
...
`format-event.ts` — pretty-prints stdout for `petagentai run --watch` using `picocolors`.
...
With these flags set, the PetAgent UI will automatically show the instructions bundle editor, skills management tab, and working directory field for agents using this adapter — no PetAgent source changes required.
...
Make PetAgent skills discoverable to your agent runtime without writing to the agent's working directory:
