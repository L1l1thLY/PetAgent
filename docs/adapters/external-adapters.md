...
summary: Build, package, and distribute adapters as plugins without modifying PetAgent source
---

PetAgent supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to PetAgent's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `petagent-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with PetAgent | Published to npm or linked via `file:` |
| Updates | Requires PetAgent release | Independent versioning |
...
```json
{
  ...
  "dependencies": {
    "@petagentai/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
| `petagent.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
```ts
import type { ServerAdapterModule } from "@petagentai/adapter-utils";
...
} from "@petagentai/adapter-utils";

import {
  runChildProcess,
  buildPetAgentEnv,
  renderTemplate,
} from "@petagentai/adapter-utils/server-utils";
...
export async function execute(
  ...
  // 2. Build environment with PetAgent vars injected
  const env = buildPetAgentEnv(agent);
...
#### Available Helpers from `@petagentai/adapter-utils`
...
```sh
# Via the PetAgent UI
# Settings → Adapters → Install from npm → "my-petagent-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-petagent-adapter"}'
```
...
Local adapters are symlinked into PetAgent's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.petagent/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-petagent-adapter",
...
```ts
import type { AdapterSessionCodec } from "@petagentai/adapter-utils";
...
Other PetAgent users can then install your adapter by package name from the UI or API.
