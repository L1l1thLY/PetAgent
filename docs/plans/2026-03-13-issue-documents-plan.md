...
Add first-class **documents** to PetAgent as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by PetAgent workflow/docs.
...
Update the PetAgent agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/petagent/SKILL.md`
