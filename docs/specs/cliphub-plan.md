# ClipHub: Marketplace for PetAgent Team Configurations
...
> The "app store" for whole-company AI teams — pre-built PetAgent configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for PetAgent-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | PetAgent company (agents, projects, governance) |
...
A complete PetAgent company configuration:
...
Single-agent configurations designed to plug into a PetAgent org:
...
Portable skill files that any PetAgent agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with PetAgent's skill loading system
...
interface Listing {
  ...
  petagentVersionMin: string;     // Minimum PetAgent version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target PetAgent company
...
| `POST` | `/api/listings/:id/install` | Install to PetAgent company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target PetAgent company (or create new)
  → ClipHub API calls PetAgent API to:
  ...
  → Redirect to PetAgent dashboard with new team running
```
...
### 5.3 Creator: Export from PetAgent → Publish

```
Running PetAgent company → "Export as Blueprint" (CLI or UI)
  → PetAgent exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (PetAgent brand blue/purple)
...
| Install flow | One-click deploy to PetAgent company |
...
{
  "targetCompanyId": "uuid",         // Existing PetAgent company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `petagent-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as PetAgent
- **Backend**: Node.js API (or extend PetAgent server)
- **Database**: Postgres (can share PetAgent's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with PetAgent auth (or OAuth2)

### 9.2 Integration with PetAgent

ClipHub can be:
- **Option A**: A separate app that calls PetAgent's API to install blueprints
- **Option B**: A built-in section of the PetAgent UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing PetAgent UI and API.
...
- [ ] Install flow: blueprint → PetAgent company
...
- [ ] "Export from PetAgent" CLI command
