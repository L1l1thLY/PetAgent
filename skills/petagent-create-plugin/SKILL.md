---
name: petagent-create-plugin
description: >
  Create new PetAgent plugins with the current alpha SDK/runtime. Use when
...
# Create a PetAgent Plugin

Use this skill when the task is to create, scaffold, or document a PetAgent plugin.
...
node packages/plugins/create-petagent-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the PetAgent repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.petagent-sdk/`:

```bash
pnpm --filter @petagentai/create-petagent-plugin build
node packages/plugins/create-petagent-plugin/dist/index.js @acme/plugin-name \
...
- is installed into PetAgent from an absolute local path during development
