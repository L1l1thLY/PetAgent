...
if (tailscaleAuth) {
  ...
  env.HOST = "0.0.0.0";
  console.log("[petagent] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@petagent/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
...
async function maybePreflightMigrations(options = {}) {
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[petagent] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). ` +
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@petagent/server", serverScript, ...forwardedArgs],
