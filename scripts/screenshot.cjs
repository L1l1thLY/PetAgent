#!/usr/bin/env node
/**
 * Screenshot utility for PetAgent UI.
 *
 * Reads the board token from ~/.petagent/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/petagent-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".petagent/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.petagent/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the PetAgent origin only
