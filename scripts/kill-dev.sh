#!/usr/bin/env bash
#
# Kill all local PetAgent dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all petagent dev processes
...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/petagent(-[^/]+)?/' | grep node | grep -v grep || true)
...
candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.petagent/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.petagent/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.petagent/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/petagent*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.petagent/instances/*/db/postmaster.pid
    "$sibling_root"/.petagent/runtime-services/instances/*/db/postmaster.pid
  )
done
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 && ${#browser_pids[@]} -eq 0 ]]; then
  echo "No PetAgent dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} PetAgent dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to PetAgent node processes..."
