...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/petagent"
  local claimed_file="${workspace_dir}/petagent-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/petagent-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/petagent-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/petagent-claimed-api-key.json" >/dev/null 2>&1 || true
  fi
