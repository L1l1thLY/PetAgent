...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-$OPENCLAW_TMP_DIR/openclaw-petagent-smoke}"
...
PETAGENT_HOST_PORT="${PETAGENT_HOST_PORT:-3100}"
PETAGENT_HOST_FROM_CONTAINER="${PETAGENT_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.petagent-openclaw.override.yml"
...
detect_petagent_base_url() {
  ...
  for candidate in "$PETAGENT_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${PETAGENT_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${PETAGENT_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
petagent_base_url="$(detect_petagent_base_url || true)"
...
PetAgent URL for OpenClaw container:
EOF
  if [[ -n "$petagent_base_url" ]]; then
    cat <<EOF
  $petagent_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${PETAGENT_HOST_PORT}
  (Do not use http://127.0.0.1:${PETAGENT_HOST_PORT} inside the container.)
  If PetAgent rejects the host, run on host machine:
    pnpm petagentai allowed-hostname host.docker.internal
  Then restart PetAgent and re-run this script.
EOF
  fi
  cat <<EOF
