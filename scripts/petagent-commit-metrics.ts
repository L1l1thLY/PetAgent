...
const DEFAULT_QUERY = "\"Co-Authored-By: PetAgent <noreply@petagent.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/petagent-commit-metrics-cache.json");
...
function printHelp() {
  console.log(`Usage: tsx scripts/petagent-commit-metrics.ts [options]
...
function printSummary(summary: Summary) {
  console.log("PetAgent commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "petagent-commit-metrics",
