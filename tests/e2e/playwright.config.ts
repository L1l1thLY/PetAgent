...
const PORT = Number(process.env.PETAGENT_E2E_PORT ?? 3199);
const BASE_URL = `http://127.0.0.1:${PORT}`;
const PETAGENT_HOME = fs.mkdtempSync(path.join(os.tmpdir(), "petagent-e2e-home-"));
...
export default defineConfig({
  ...
  // `onboard --yes --run` works in a non-interactive temp PETAGENT_HOME.
  webServer: {
    ...
    env: {
      ...
      PORT: String(PORT),
      PETAGENT_HOME,
      PETAGENT_INSTANCE_ID: "playwright-e2e",
      PETAGENT_BIND: "loopback",
      PETAGENT_DEPLOYMENT_MODE: "local_trusted",
