...
Low-level reference for how PetAgent packages are prepared and published to npm.
...
PetAgent no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `petagentai`, imports code from workspace packages such as:
...
### Manual first publish for `@petagentai/ui`
...
npm view @petagentai/ui version

# make sure the dist payload is fresh
pnpm --filter @petagentai/ui build
...
- If `npm view @petagentai/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@petagentai/ui`, the `petagentai` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
PetAgent uses calendar versions:
...
- `petagentai@2026.318.1-canary.2`

This keeps the default install path unchanged while allowing explicit installs with:

```bash
npx petagentai@canary onboard
```
