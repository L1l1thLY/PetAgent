...
This document explains how PetAgent interprets issue assignment, issue status, execution runs, wakeups, parent/sub-issue structure, and blocker relationships.

`doc/SPEC-implementation.md` remains the V1 contract. This document is the detailed execution model behind that contract.

## 1. Core Model

PetAgent separates four concepts that are easy to blur together:
...
This is a hard invariant. PetAgent is single-assignee by design.

## 3. Status Semantics

PetAgent issue statuses are not just UI labels. They imply different expectations about ownership and execution.
...
- for agent-assigned work, PetAgent may still need a wake path to ensure the assignee actually sees it
...
- PetAgent can wake the assignee
- PetAgent can track runs linked to the issue
- PetAgent can recover some lost execution state after crashes/restarts

### User-owned issues

User-owned issues are not executed by the heartbeat scheduler.

- PetAgent can track the ownership and status
- PetAgent cannot rely on heartbeat/run semantics to keep them moving
...
PetAgent already clears stale execution locks and can adopt some stale checkout locks when the original run is gone.

## 6. Parent/Sub-Issue vs Blockers

PetAgent uses two different relationships for different jobs.
...
For agent-assigned, non-terminal, actionable issues, PetAgent should not leave work in a state where nobody is working it and nothing will wake it.
...
PetAgent now treats crash/restart recovery as a stranded-assigned-work problem, not just a stranded-run problem.
...
- if the latest issue-linked run failed/timed out/cancelled and no live execution path remains, PetAgent queues one automatic assignment recovery wake
- if that recovery wake also finishes and the issue is still stranded, PetAgent moves the issue to `blocked` and posts a visible comment
...
- PetAgent queues one automatic continuation wake
- if that continuation wake also finishes and the issue is still stranded, PetAgent moves the issue to `blocked` and posts a visible comment
...
On startup and on the periodic recovery loop, PetAgent now does three things in sequence:
...
That is the execution contract PetAgent should present to operators.
