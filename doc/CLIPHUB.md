...
ClipHub is the public registry where people share, discover, and download PetAgent company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to PetAgent what a package registry is to a programming language. PetAgent already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in PetAgent — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own PetAgent instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the PetAgent spec. It contains:
...
**Install (fresh start):**
```
petagent install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local PetAgent instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within PetAgent, export your company as a template, then publish:

```
petagent export --template my-company
petagent publish cliphub my-company
```
...
ClipHub is a **separate service** from PetAgent itself. PetAgent is self-hosted; ClipHub is a hosted registry that PetAgent instances talk to.
...
| **PetAgent CLI** | `petagentai install`, `petagentai publish`, `petagentai cliphub sync` — built into PetAgent |
| **PetAgent UI** | "Browse ClipHub" panel in the PetAgent web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with PetAgent) |
| Backend | TypeScript + Hono (consistent with PetAgent) |
| Database | PostgreSQL |
...
4. Run `petagentai install cliphub:acme/lean-saas-shop`
5. PetAgent creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in PetAgent until it works well
2. Export: `petagentai export --template my-agency`
3. Publish: `petagentai publish cliphub my-agency`
...
3. Install just that agent: `petagentai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to PetAgent

ClipHub is **not required** to use PetAgent. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to PetAgent what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `petagentai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in PetAgent web UI
- [ ] `petagentai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with PetAgent's own philosophy)
