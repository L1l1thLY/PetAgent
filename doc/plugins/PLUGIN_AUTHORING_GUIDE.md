...
This guide describes the current, implemented way to create a PetAgent plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main PetAgent app.
...
```bash
pnpm --filter @petagentai/create-petagent-plugin build
node packages/plugins/create-petagent-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the PetAgent repo:

```bash
pnpm --filter @petagentai/create-petagent-plugin build
node packages/plugins/create-petagent-plugin/dist/index.js @yourscope/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/petagent/packages/plugins/sdk
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@petagentai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@petagentai/plugin-sdk` from the local PetAgent checkout into a `.petagent-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into PetAgent from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@petagentai/plugin-sdk/ui`
