# PetAgent Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for PetAgent's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main PetAgent app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary PetAgent HTTP APIs directly.
...
PetAgent plugin design is based on the following assumptions:

1. PetAgent is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core PetAgent business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by PetAgent core.
...
2. Let plugins add major capabilities without editing PetAgent core.
...
4. Depend on project-local plugin folders such as `.petagent/plugins`.
...
The single PetAgent deployment an operator installs and controls.

### 5.2 Company

A first-class PetAgent business object inside the instance.
...
A trusted in-process extension loaded directly by PetAgent core.
...
An installable instance-wide extension package loaded through the PetAgent plugin runtime.
...
PetAgent has two extension classes.
...
PetAgent already has a concrete workspace model:
...
Plugins live under the PetAgent instance directory.

Suggested layout:

- `~/.petagent/instances/default/plugins/package.json`
- `~/.petagent/instances/default/plugins/node_modules/`
- `~/.petagent/instances/default/plugins/.cache/`
- `~/.petagent/instances/default/data/plugins/<plugin-id>/`
...
PetAgent should add CLI commands:

- `pnpm petagentai plugin list`
- `pnpm petagentai plugin install <package[@version]>`
- `pnpm petagentai plugin uninstall <plugin-id>`
- `pnpm petagentai plugin upgrade <plugin-id> [version]`
- `pnpm petagentai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@petagent/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface PetAgentPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that PetAgent agents can use during runs.
...
- PetAgent server starts one worker process per installed plugin
...
Receives one typed PetAgent domain event.
...
export function definePlugin(definition: PluginDefinition): PetAgentPlugin;
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@petagentai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@petagentai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@petagentai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@petagentai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@petagentai/plugin-sdk/ui`
...
The `@petagentai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the PetAgent secret provider system rather than a plain text input
...
1. Core PetAgent data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing PetAgent objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual PetAgent product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if PetAgent later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing PetAgent secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm petagentai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the PetAgent server. This is a normative requirement, not optional.
...
### 27.1 `@petagentai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@petagentai/plugin-test-harness";
...
For developing a plugin against a running PetAgent instance:

- The operator installs the plugin from a local path: `pnpm petagentai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-petagent-plugin`) that scaffolds:

- `package.json` with correct `petagentPlugin` keys
...
- `@petagent/plugin-workspace-files`
- `@petagent/plugin-terminal`
- `@petagent/plugin-git`
- `@petagent/plugin-linear`
- `@petagent/plugin-github-issues`
- `@petagent/plugin-grafana`
- `@petagent/plugin-runtime-processes`
- `@petagent/plugin-stripe`
...
- `@petagentai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@petagentai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@petagentai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@petagentai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@petagentai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@petagentai/plugin-sdk/ui`
...
- `@petagentai/plugin-test-harness`
- `create-petagent-plugin` starter template
...
PetAgent should not implement a generic in-process hook bag modeled directly after local coding tools.

PetAgent should implement:
...
That is the complete target design for the PetAgent plugin system.
