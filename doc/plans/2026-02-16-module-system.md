...
PetAgent's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends PetAgent's API, UI, and data model | Yes |
...
modules/
  observability/
    petagent.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@petagentai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@petagentai/core";
...
import type { Db } from "@petagentai/db";
...
```
1. Scan modules/ directory for petagent.module.json manifests
...
```jsonc
// petagent.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@petagentai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@petagentai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:petagent/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.petagent.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm petagentai store list                    # browse available modules and templates
pnpm petagentai store install <module-id>     # install a module
pnpm petagentai store import <template-id>    # import a company template
pnpm petagentai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one PetAgent instance | `server:started` |
...
4. **Module config** — `petagent.config.json` with per-module config, env var interpolation
...
10. **`@petagentai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
