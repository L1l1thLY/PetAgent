...
PetAgent already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model PetAgent should implement next.

## Product Goals

PetAgent should let operators:
...
For the next implementation pass, PetAgent should enforce these defaults:
...
PetAgent should separate subscription-included usage from billed spend:
...
PetAgent should have two threshold classes:
...
PetAgent must know that a project is budget-paused and enforce it.
...
PetAgent must also block execution before new work starts.
