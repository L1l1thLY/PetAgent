...
Agents need API keys to authenticate with PetAgent. The current approach
...
**Trust model:** The adapter process runs on the same machine as the PetAgent
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** PetAgent generates a token and passes it directly to the agent
...
`PETAGENT_API_URL`, `PETAGENT_AGENT_ID`, `PETAGENT_COMPANY_ID`. We need to
add a `PETAGENT_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a PetAgent CLI
...
```
petagent auth login
# Opens browser -> user confirms -> token stored at ~/.petagent/credentials
```
...
managed by the PetAgent server itself.
...
1. A company admin (user or agent) generates an **invite URL** from PetAgent.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The PetAgent SKILL.md (or a link to it)
   - What information PetAgent needs from the agent (e.g. webhook URL, adapter
...
5. PetAgent stores the pending registration.
...
7. On approval, PetAgent provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** PetAgent issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.petagent.ing/skills/petagent/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How PetAgent should send you heartbeats",
...
1. Generate a PetAgent invite link for the company.
...
4. A PetAgent company member approves the new agent.
5. PetAgent begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `PETAGENT_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `petagentai auth login` for developer-managed remote agents.                                      |
