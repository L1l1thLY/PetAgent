...
We were asked to find the hook surfaces exposed by `pi` and `pi-mono`, then decide which ideas transfer cleanly into PetAgent.
...
- current PetAgent plugin and adapter surfaces in this repo
...
## What PetAgent already has

PetAgent already has several hook-like surfaces, but they are much narrower and safer:
...
## What transfers well to PetAgent

These ideas from `pi-mono` fit PetAgent with little conceptual risk:

### 1. Read-only run lifecycle subscriptions

PetAgent should continue exposing run and transcript events to plugins, for example:
...
This matches PetAgent's control-plane posture: observe, react, automate.

### 2. Plugin-to-plugin events

PetAgent already has this. It is worth keeping and extending.

This is the clean replacement for many ad hoc hook chains.

### 3. Explicit worker lifecycle hooks

PetAgent already has `setup()` and `onHealth()`. That is the right shape.

If more lifecycle is needed, it should stay explicit and host-controlled.

### 4. Trusted adapter-level prompt/runtime middleware

Some `pi-mono` ideas do belong in PetAgent, but only inside trusted adapter/runtime code:
...
These `pi-mono` capabilities are a bad fit for PetAgent core:

### 1. Arbitrary mutation hooks on control-plane decisions

PetAgent should not let general plugins rewrite:
...
PetAgent should keep plugin tools namespaced and non-shadowing.

### 3. Project-local plugin loading

PetAgent is an operator-controlled control plane. Repo-local plugin auto-loading would make behavior too implicit and too hard to govern.
...
They do not map directly to PetAgent's board-and-issues model.

## Recommended PetAgent direction
...
If the question is "what works for PetAgent too?":
