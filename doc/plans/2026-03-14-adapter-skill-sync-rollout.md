...
This document defines the rollout plan for adapter-wide skill support in PetAgent.
...
- PetAgent stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

PetAgent currently has these adapters:
...
1. Desired skills live in PetAgent for every adapter.
...
These adapters have a stable local skills directory that PetAgent can read and manage.
...
These adapters do not have a meaningful PetAgent-owned persistent install state.
...
- desired skills stored in PetAgent
...
- runtime already injects PetAgent skills into `~/.cursor/skills`
...
- runtime already injects PetAgent skills into `~/.gemini/skills`
...
- runtime already injects PetAgent skills into `~/.pi/agent/skills`
...
- PetAgent can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects PetAgent skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- PetAgent may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed PetAgent skills that are clearly marked as PetAgent-managed
...
- PetAgent stores desired skills only
...
- avoid destructive wording unless PetAgent can prove a skill is PetAgent-managed
...
That gets PetAgent from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
