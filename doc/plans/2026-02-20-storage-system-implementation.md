...
Add a single storage subsystem for PetAgent that supports:
...
- API serves attachment bytes through authenticated PetAgent endpoints.
...
- Default local path is under instance root: `~/.petagent/instances/<instanceId>/data/storage`.
...
- `petagentai onboard` writes a valid `storage` config block by default.
- `petagentai configure --section storage` can switch between local and s3 modes.
...
- `petagentai doctor` reports actionable storage status.
