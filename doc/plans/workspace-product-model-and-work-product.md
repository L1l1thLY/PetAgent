...
PetAgent needs to support two very different but equally valid ways of working:
...
Today, PetAgent already has the beginnings of this model:
...
- making PetAgent feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing PetAgent users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted PetAgent deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning PetAgent into a full code review product
...
This object must be recorded explicitly so that PetAgent can:
...
PetAgent should treat PRs as a type of work product linked back to:
...
PetAgent already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted PetAgent must be a first-class deployment mode

PetAgent cannot assume that it is running on the same machine as the code.

In cloud deployments, PetAgent may:
...
- `Runtime service`: a process or service PetAgent owns or tracks for a workspace
...
It also lets cloud-hosted PetAgent deployments point at codebases and remotes without pretending the PetAgent host has direct filesystem access.
...
Without an explicit `execution workspace` record, PetAgent has nowhere stable to attach:
...
PetAgent needs a single place to show:
...
- PetAgent-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without PetAgent owning the execution host
...
## 1. PetAgent host and execution host must be decoupled
...
without the PetAgent host itself running local git or local preview processes.
...
PetAgent should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for PetAgent.
...
PetAgent should keep workspace automation centered on a small set of hooks:
...
PetAgent should provide the equivalent visibility in the project `Code` surface:
...
PetAgent should treat execution workspaces as the natural home for:
...
PetAgent should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
PetAgent should continue treating cleanup policy as part of the core workspace model:
...
PetAgent should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the PetAgent host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

PetAgent should borrow Worktrunk's good execution patterns while keeping the broader PetAgent model:
...
That keeps PetAgent focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted PetAgent deployments remain first-class

That is a better fit for PetAgent than either extreme:
