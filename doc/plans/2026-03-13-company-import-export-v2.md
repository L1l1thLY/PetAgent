...
This document defines the next-stage plan for PetAgent company import/export.

The core shift is:

- move from a PetAgent-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside PetAgent.
...
PetAgent already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just PetAgent
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `petagent.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.petagent.yaml` sidecar for high-fidelity PetAgent-specific details
...
11. adapter-aware skill sync surfaces so PetAgent can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point PetAgent at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a PetAgent sidecar.
...
PetAgent must not redefine `SKILL.md`.
...
- PetAgent-specific fidelity lives in `.petagent.yaml`
- PetAgent may resolve and install `SKILL.md` packages, but it must not require a PetAgent-only skill format
...
### 5.4 Base Package Vs PetAgent Extension
...
- base package:
  ...
  - no PetAgent-only runtime fields by default
- PetAgent extension:
  - `.petagent.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by PetAgent tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`petagent.manifest.json` is not part of the future package direction.
...
PetAgent import/export should support these entity kinds:
...
- PetAgent can treat “this set of agents came from team package X” as the imported-team model
...
PetAgent should support source references in package metadata with:
...
PetAgent should define a new adapter surface area around skills:
...
Planned PetAgent behavior:

- if an adapter supports read, PetAgent should show current skills in the UI
- if an adapter supports write, PetAgent should let the user enable/disable imported skills
- if an adapter supports sync, PetAgent should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.petagent.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.petagent.yaml` alongside the base package
...
## 10. Storage Model Inside PetAgent
...
PetAgent should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `petagentai company export <company-id> --out <path>`
- `petagentai company import <path-or-url> --dry-run`
- `petagentai company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in PetAgent storage, or only referenced at import time?
...
3. Should PetAgent support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps PetAgent aligned with:
