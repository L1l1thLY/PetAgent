...
`PAP-447` asks how PetAgent should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same PetAgent instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into PetAgent:

- some operators use PetAgent to manage PetAgent and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as PetAgent
- Claude and Codex expose different built-in affordances, so PetAgent should not overfit to one tool

## Core Product Decision

PetAgent should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by PetAgent
...
- PetAgent should not assume that every adapter can see or use a host filesystem path directly
...
If PetAgent makes worktrees agent-first, it will blur:
...
- PetAgent core resolves and records execution workspace state
...
That gives PetAgent room to support:
...
- PetAgent handles workspace runtime services under the hood using project/workspace policy
...
Once PetAgent is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So PetAgent should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that PetAgent cannot infer
...
- when issue moves to done, should PetAgent auto-commit?
- should it auto-push?
...
PetAgent already has the right foundation for a project-first model.
...
For server-spawned local adapters, PetAgent already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that PetAgent should mirror directly

Implication:

- for `codex_local`, PetAgent should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but PetAgent should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, PetAgent should separate:
...
PetAgent should be able to express intentions such as:
...
These are not specific to PetAgent. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".petagent/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a PetAgent-specific `devServer` flag.
...
PetAgent should distinguish between:
...
PetAgent should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, PetAgent should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to PetAgent
...
For runtime services specifically, PetAgent should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as PetAgent-only dev-server flags
...
3. Inject resolved execution workspace details into `context.petagentWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / PetAgent instance can be reused through env/config injection when appropriate
...
- PetAgent can decide whether to reuse or start a fresh service deterministically
...
3. Test against PetAgent-on-PetAgent workflows first.
...
7. Workspace runtime services are modeled generically, not as PetAgent-specific dev-server toggles.
