...
This document defines the V1 design for a PetAgent adapter that integrates with
...
4. synthesized stdout events for PetAgent UI/CLI
...
3. **Agent callback auth to PetAgent** uses a bootstrap exchange flow (no long-lived PetAgent key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from PetAgent endpoints, not full SKILL.md prompt injection.
...
- `petagentPublicUrl` (optional override; else `PETAGENT_PUBLIC_URL` env)
...
## PetAgent Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `PETAGENT_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.petagentPublicUrl`
2. `process.env.PETAGENT_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived PetAgent credentials in prompt text.

Flow:

1. Before launch/follow-up, PetAgent mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. PetAgent validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all PetAgent API calls.
...
1. Prompt includes a compact instruction to fetch skills from PetAgent.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/petagent`
   - `GET /api/skills/petagent-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <petagentPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active PetAgent run context.
...
5. `petagentPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from PetAgent skill endpoints |
| PetAgent auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from PetAgent context.
3. Add richer PR metadata surfacing in PetAgent UI.
