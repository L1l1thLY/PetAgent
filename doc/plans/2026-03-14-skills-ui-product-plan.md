...
This document defines the product and UI plan for skill management in PetAgent.
...
- PetAgent company import/export can include skills as package content
...
3. The UI must always tell the truth about what PetAgent knows:
   - desired state in PetAgent
...
PetAgent should treat skills at two scopes:
...
- created locally inside PetAgent later
...
7. “Understand what PetAgent knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a PetAgent-specific package layout
...
This matters because Codex and similar adapters may already have local skills that PetAgent did not install.

These should be clearly marked:

- external
- not managed by PetAgent
...
- `desired_only`: PetAgent wants it, adapter does not show it yet
- `external`: adapter has it but PetAgent does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, PetAgent only tracks desired state
...
- PetAgent can still track desired skills
...
- PetAgent can see adapter skills
...
- `.petagent.yaml` may add PetAgent-specific fidelity, but should not replace the base shortname association model
...
- `.petagent.yaml` may add PetAgent-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
PetAgent should have a company-scoped skill model or managed package model representing:
...
PetAgent should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too PetAgent-specific.
...
That gives PetAgent one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
