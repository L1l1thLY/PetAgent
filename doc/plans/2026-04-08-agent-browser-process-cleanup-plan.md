...
Yes, there is a likely root cause in PetAgent's local execution model.
...
If Codex, Claude, Cursor, or a skill launched through them starts Chrome or Chromium helpers, PetAgent can lose ownership of those descendants even when it still believes it handled the run correctly.
...
This is the clearest internal comparison point: PetAgent already has one local-process subsystem that treats process-group ownership as the right abstraction.
...
- PetAgent may think it cancelled the run because the parent process is gone
...
This work is successful when PetAgent can:
...
- require every skill author to add bespoke browser teardown logic before PetAgent can clean up correctly
...
- make the leak measurable from PetAgent's side before changing execution semantics
...
- add a control-plane cleanup action or CLI utility for stale local run processes owned by PetAgent
...
Treat this as a PetAgent executor ownership bug, not an `agent-browser` bug.
