...
We need evals for the thing PetAgent actually ships:
...
- can we grow the suite over time from real PetAgent usage?
...
- Discussion #449: <https://github.com/petagentai/petagent/discussions/449>
...
PetAgent should take a **two-stage approach**:

1. **Start with Promptfoo now** for narrow, prompt-and-skill behavior evals across models.
2. **Grow toward a first-party, repo-local eval harness in TypeScript** for full PetAgent scenario evals.
...
3. The longer-term harness should run **real PetAgent scenarios** against seeded companies/issues/agents, not just raw prompt completions.
...
That is the right unit because that is what actually changes behavior in PetAgent.

## Why This Is The Right Shape

### 1. We need to evaluate system behavior, not only prompt output

Prompt-only tools are useful, but PetAgent’s real failure modes are often:
...
But OpenAI’s Evals API is not the right control plane for PetAgent as the primary system because our target is explicitly multi-model and multi-provider.
...
But PetAgent should still avoid making a hosted platform or a third-party config format the core abstraction before we have our own stable eval model.

The right move is:

- start with Promptfoo for quick wins
- keep the data portable and repo-owned
- build a thin first-party harness around PetAgent concepts as the system grows
...
These are the closest thing to prompt evals, but still framed in PetAgent terms.
...
The important part is that the case is about a PetAgent scenario, not a standalone prompt string.
...
PetAgent should grow eval coverage by promoting real runs into cases when we see:
...
- PetAgent needs scenario setup, control-plane state inspection, and multi-step traces as first-class concepts

Recommendation:

- use Promptfoo first
- store Promptfoo config and cases in-repo under `evals/promptfoo/`
- use custom JS/TS assertions and, if needed later, a custom provider that calls PetAgent scenario runners
- do not make Promptfoo YAML the only canonical PetAgent eval format once we outgrow prompt-level evals
...
- PetAgent must compare across models/providers
- we do not want our primary eval runner coupled to one model vendor

Recommendation:

- use the guidance
- do not use it as the core PetAgent eval runtime
...
- did they call the right PetAgent APIs?
...
Start with Promptfoo for immediate, narrow model-and-prompt comparisons, then grow into a first-party `evals/` framework in TypeScript that evaluates **PetAgent scenarios and bundles**, not just prompts.
...
But keep the canonical eval model inside the PetAgent repo and aligned to PetAgent’s actual control-plane behaviors.
