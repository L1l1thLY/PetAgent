...
Turn the `agent-os` research into a low-risk PetAgent execution plan that preserves PetAgent's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

PetAgent should not absorb `agent-os` as a product model or orchestration layer.

PetAgent should evaluate `agent-os` in three narrow areas:
...
This work is successful when PetAgent has:
...
- a proposed runtime capability model that fits current PetAgent adapters
...
- replace PetAgent heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to PetAgent core

## Existing PetAgent integration points
...
- prove that PetAgent can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep PetAgent's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a PetAgent-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some PetAgent workloads
...
- identify where PetAgent should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in PetAgent
...
- runtime experimentation could blur the boundary between PetAgent as control plane and PetAgent as execution platform
