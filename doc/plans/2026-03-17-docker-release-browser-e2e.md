...
Today release smoke testing for published PetAgent packages is manual and shell-driven:
...
- Docker
- `npx petagentai@canary`
- `npx petagentai@latest`
...
- runs `petagentai@${PAPERCLIPAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@petagent.local`
- password: `petagent-smoke-password`
...
1. A published `petagentai@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `petagentai@latest` Docker install can be smoke-tested by Playwright in CI.
