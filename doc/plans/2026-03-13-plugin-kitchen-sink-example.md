...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented PetAgent plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside PetAgent.
...
- package: `@petagentai/plugin-kitchen-sink-example`
- plugin id: `petagent.kitchen-sink-example` or `petagent-kitchen-sink-example`

Recommendation: use `petagent-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core PetAgent event such as `issue.created`
...
Recommendation: default to a PetAgent-local endpoint or a stable public echo endpoint to avoid flaky docs.
