...
This spec defines how PetAgent actually runs agents while staying runtime-agnostic.
...
1. PetAgent is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, PetAgent manages lifecycle, and on exit PetAgent parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `petagent.skill` (shared PetAgent skill text block)
...
2. Preferred transport is env vars (`PETAGENT_*`) injected at runtime.
