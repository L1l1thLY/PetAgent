...
For `claude_local` and `codex_local`, PetAgent assumes the CLI is already installed and authenticated on the host machine.
...
PetAgent stores resumable session state per `(agent, taskKey, adapterType)`.
