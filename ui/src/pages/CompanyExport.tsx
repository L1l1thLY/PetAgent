...
/**
 * Filter .petagent.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterPetAgentYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://petagent.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm petagent company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [PetAgent](https://petagent.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [PetAgent](https://petagent.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .petagent.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .petagent.yaml
    const yamlPath = exportData.petagentExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterPetAgentYaml(exportData.files[yamlPath], checkedFiles);
    }
