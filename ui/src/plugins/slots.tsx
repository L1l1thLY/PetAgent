 ...
 *
 * Plugin bundles are built with `external: ["@petagent/plugin-sdk/ui", "react", "react-dom"]`,
 * so their ESM output contains bare specifier imports like:
 *
 * ```js
 * import { usePluginData } from "@petagent/plugin-sdk/ui";
 ...
 *    host's global bridge registry (`globalThis.__petagentPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__petagentPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__petagentPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__petagentPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@petagent/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@petagent/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@petagent/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@petagent/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@petagent/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__petagentPluginBridge__) {
 ...
 * self-register via `window.petagentPlugins.registerReactComponent()`.
 ...
 * of `@petagent/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
