...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("petagent-worktree-enabled") !== "true") return null;

  const name = readMetaContent("petagent-worktree-name");
  const color = normalizeHexColor(readMetaContent("petagent-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("petagent-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
